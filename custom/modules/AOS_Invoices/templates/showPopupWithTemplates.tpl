...
<div id="popupDiv_ara_custom"
     style="display:none;position:fixed;top: 39%; left: 41%;opacity:1;z-index:9999;background:#FFFFFF;">
    <form id="popupFormCustom" action="index.php?entryPoint=customGeneratePdf" method="post">
        <table style="border: #000 solid 2px; padding-left:40px; padding-right:40px; padding-top:10px; padding-bottom:10px; font-size:110%;">
            <tr height="20">
                <td colspan="2">
                    <b>{$APP.LBL_SELECT_TEMPLATE}:-</b>
            ...
            {if empty($template) == false}
            {capture name=on_click_js assign=on_click_js}
            document.getElementById('popupDivBack_ara_custom').style.display='none';document.getElementById('popupDiv_ara_custom').style.display='none';var form=document.getElementById('popupFormCustom');if(form!=null){ldelim}form.templateID.value='{$template}';form.submit();{rdelim}else{ldelim}alert('Error!');{rdelim}
            {/capture}
            <tr height="20">
                <td width="17" valign="center"><a href="#" onclick="{$on_click_js}">
                        <a href="#" onclick="{$on_click_js}">
    ...
    <tr>
        <td colspan="2">
            <button style=" display: block;margin-left: auto;margin-right: auto" onclick="document.getElementById('popupDivBack_ara_custom').style.display='none';document.getElementById('popupDiv_ara_custom').style.display='none';return false;">
                Cancel
            </button>
        </td>
    </tr>
...
<div id="popupDivBack_ara_custom" onclick="this.style.display='none';document.getElementById('popupDiv_ara_custom').style.display='none';" style="top:0px;left:0px;position:fixed;height:100%;width:100%;background:#000000;opacity:0.5;display:none;vertical-align:middle;text-align:center;z-index:9998;">
</div>
...
<script>
   ...
   */
  {/literal}
    function showPopupCustom(task) {ldelim}
    var form = document.getElementById('popupFormCustom');
    var ppd = document.getElementById('popupDivBack_ara_custom');
    var ppd2 = document.getElementById('popupDiv_ara_custom');
    var totalTemplates = {$TOTAL_TEMPLATES}
    if (totalTemplates === 1) {ldelim}
    form.task.value = task;
    form.templateID.value = '{$template}';
